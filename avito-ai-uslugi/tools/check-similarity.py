...
Использование:
    py -3 tools/check-similarity.py

Адаптировано из avito-toolkit/05-similarity-checker.py (методология frezorez) —
по инструкции толкита скрипт переиспользуется без изменения логики, меняется
только блок "НАСТРОЙКИ" под расположение листингов этого проекта
(prompts/listings/, маркеры ## ОПИСАНИЕ / ## ХАРАКТЕРИСТИКИ — те же, см.
_split_listings вывод и шаблон в prompts/listings/*.md).
...
# ───────────────────────── НАСТРОЙКИ ─────────────────────────
LISTINGS_DIR = Path(__file__).parent.parent / "prompts" / "listings"
THRESHOLD = 0.40
